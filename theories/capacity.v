...
Section Capacity.
  ...
  Section CapaDefs.
    ...
    (* Note: the following propositions are equivalent :
       - f is monotone wrt inclusion
       - A \subset B -> f A <= f B
       - A \subset B -> \sum_(i \subset A) m_f i <= \sum_(i \subset B) m_f i
       - f A <= f (A :|: B)
       - \sum_(i \subset A) m_f i <= \sum_(i \subset A :|: B) m_f i
     *)
    (*
    Definition monotonic_massfun (m : {ffun {set W} -> R}) :=
      [forall A : {set W}, [forall B : {set W}, \sum_(i : {set W} | i \subset A) m i <= \sum_(i : {set W} | i \subset A :|: B) m i]].
     *)
    
    (* TODO: monotony is missing *)
...
*)
  ...
  Section BelTypes.
    ...
    Definition massfun_eqMixin := EqMixin massfun_eqP.
    Canonical massfun_eqType := EqType massfun massfun_eqMixin.
  ...
  *)
  ...
  Section ScoringFunctions.
    ...
    Definition XEU (m : massfun) (f_xeu : {set W} -> R) : R
  ...
  Section TBM.
    ...
    Lemma TBEU_E (m : massfun) u :
      XEU m (TBEU u) = \sum_w BetP_fun m w * u w.
    Proof.
    rewrite/XEU sum_fun_focalset /=.
...
Notation "k '.-additive' m" := (k_additivity m == k) (at level 80, format " k '.-additive'  m ").
Notation proba_axiom p := (1%N.-additive p) (only parsing).
Notation "'xeu_fun_type' R W" := ((W -> R) -> {set W} -> R) (at level 80).
